<?php 
	...
	if (isset($_POST['insert-bus'])) {
		...
		if ($admin=="" || $category=="" || $source=="" || $destination=="" || $title=="" || $date=="" || $bus_detail=="" || $max_seats=="") {
			echo "**All Fields Mandatory";
		}
		else {
			$query = "INSERT INTO posts(post_category_id, post_title, post_author, post_date, post_image, post_content, post_source, post_destination, max_seats, available_seats) VALUES({$category}, '{$title}', '{$admin}', '{$date}', '{$image}', '{$bus_detail}', '{$source}', '{$destination}', '{$max_seats}')";
